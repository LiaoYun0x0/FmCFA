...
def main(args):
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.n_epoch, eta_min=1e-6)
    ...
    best_loss = 20000
    writer = SummaryWriter(f'logs/loss{str(time.time())}')
    for epoch in range(args.n_epoch):
        ...
        writer.add_scalar('loss', train_stats['losses'], global_step=epoch)
        if epoch % 20 == 0:
            scheduler.step()
        ...
        if log_stats['train_losses'] < best_loss:
            ...
            torch.save({'model': model_without_ddp.state_dict()}, f'{save_path}/model_SE_rgb150_best_{str(round(best_loss, 3))}.pth')
        with open(f'{save_path}/train_SE_rgb150_nocontri.log', 'a') as f:
